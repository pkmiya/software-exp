...
calculator:
	/* Init valiables */
	move.l	#0, ope1        /* 1st operand */
	move.b	#0, ope2        /* operator */
	move.l	#0, ope3        /* 2nd operand */
	move.l	#0, ans_print   /* print buffer */
	move.l	#0, buf_len     /* input buffer length */

    /* Process 1 */
	jsr	get_expr

	move.l #SYSCALL_NUM_PUTSTRING,%D0
	move.l #0, %d1          /* init %d1 for later */
    move.l #CRLF, %d2       /* display enter */
    move.l #2, %d3          /* size = 2 */
    trap #0                 /* syscall */
...
** CALCULATOR MAIN PROGRAM | v1.0 2023-11-14
** EXCEPT FOR INTERFACE; ALL FUNCTIONS ARE IMPLEMENTED IN MAIN SUBROUTINES

/* STORE INPUT EXPRESSION IN BUFFER */
** When character entered, store characters in buffer using GETSTRING and display characters using PUTSTRING, both one by one
** When Enter entered, go to next process
get_expr:
    movem.l	%a1-%a2/%d0-%d3, -(%sp)     /* START: save registers */
    lea.l	buf, %a1                    /* get head address of buf */
    lea.l	buf, %a2
    move.l	#0, %d1
    move.l	%a1, %d2                    /* copy head address for later */
    move.l	#1, %d3
...
get_expr_1_loop:
    ...
    /* Loop: wait until receive input */
	cmp.l	#0, %d0
	beq	get_expr_1_loop 	 

    /* Loop: wait for expression end = Enter */
	move.l  %d2, %a1
    cmp.b   #0x0d, (%a1)                /* see if the current character is Enter */
    beq     get_expr_2_end
...
get_expr_2_end:
    /* count and store exp length */
	add.w   #1, %d2
	move.l	%d2, buf_len
	movem.l	(%sp)+, %a1-%a2/%d0-%d3     /* END: restore registers */
	rts


/* DEVIDE EXPRESSIONS INTO TOKENS */
** Refer to the comments in each label
** Operand: store operand value in %d0; calc and store operand by %d0 = %d0 * 10 + (input - '0')
** Operator: store operator value in %d1

/* Initialize registers */
token_div:
	movem.l %a1-%a2/%d0-%d2, -(%sp)
	move.b	#0, %d0                     /* ope1: 1st operand */
	move.b	#0, %d1                     /* ope2: 2nd operand */
	move.b	#0, %d2                     /* ope3: operator    */
	lea.l   buf, %a1                    /* init buffer */
	movea.l	#buf_len, %a2

/* 1st operand analysis - DO THE SAME FOR 2ND OPERAND */
token_div_1_operand_1:
    /* see if the current character is number (0-9) */
	cmp.b	#'0', (%a1)
	bcs	token_div_2_operator
	cmp.b	#'9', (%a1)
	bhi	token_div_2_operator

    /* true in the above condition; calc and store operand*/
	mulu	#10, %d0
	sub.b	#'0', (%a1)
	add.b	(%a1), %d0
	adda.w	#1, %a1

    /* see if the current position is the end of the expression */
	cmp.l	%a2, %a1
	beq	token_div_4_end
	bra	token_div_1_operand_1

/* Operator analysis */
token_div_2_operator:
    /* JUST store the operator and inc pointer*/
	move.b	(%a1), %d2
	adda.w  #1, %a1

/* 2nd operand analysis - DO THE SAME FOR 1ST OPERAND */
token_div_3_operand_2:
    /* see if the current character is number (0-9) */
    cmp.b   #'0', (%a1)
    bcs  	token_div_4_end
    cmp.b   #'9', (%a1)
    ...
    cmp.l   %a2, %a1
    beq     token_div_4_end
    bra     token_div_3_operand_2	

/* Store data */
token_div_4_end:
    /* store operands and operator in global variables */
	move.l	%d0, ope1
	move.l	%d1, ope3
	move.l	%d2, ope2
...
** Store the result in %d3 and global variable ans
calc:
    movem.l %d0-%d3, -(%sp)         /* START: save registers */
    move.l  ope1, %d0
    move.l  ope3, %d1
    move.l  ope2, %d2
    move.l  #0, %d3
...
calc_4_div:
    move.l  %d0, %d3          
    divu.w  %d1, %d3                /* Upper word: remainder, lower word: quotient */
    andi.l  #0x0000ffff, %d3        /* Store only quotient by logical proc */
    move.l  %d3, ans

calc_5_end:
    movem.l (%sp)+, %d0-%d3         /* END: restore registers */
    rts


/* CONVERT AND DISPLAY DATA */
** Convert the result into decimal following the ASCII code
** Display the result using PUTSTRING
print:
    movem.l %a1/%d0-%d5, -(%sp)     /* START OF PRINT: save registers */
    lea.l   ans_print, %a1          /* print buffer */
    move.l  ans, %d0                /* recover result */
    move.w  #0, %d1                 /* calc: store temp data */
    move.w  #0, %d2                 /* calc: count digit */

print_1_calc:
    /* push answer into stack */
    divu.w  #10, %d0
    move.l  %d0, %d1                /* copy ans for later use */
    andi.l   #0x0000ffff, %d0       /* to store only quotient (1/2) */
    swap    %d1                     /* to store only quotient (2/2) */
    move.b  %d1, -(%sp)             /* push digits, starting from the first place of the number */

    /* count digit */
    add.w   #1, %d2
    cmp.w   #0, %d0                 /* see if ans is fully stored */
    bne     print_1_calc
    move.w  %d2, %d5                /* copy digit count for later */

print_2_loop:
    /* pop answer from stack */
    cmp.w   #0, %d2                 /* see if ans is fully pulled */    
    beq     print_3_end
    move.b  (%sp)+, (%a1)           /* pop digits, starting from the last place of the number */
    add.b   #'0', (%a1)+            /* convert to ASCII code */
    sub.w   #1, %d2                 /* count digit */
    bra     print_2_loop
...
print_3_end:
    ...
    move.l  #SYSCALL_NUM_PUTSTRING, %d0
    move.l  #0, %d1                 /* init %d1 for later */
    move.l  %d5, %d3                /* store no. of digits for later */
    move.l  #ans_print, %d2         /* display buffer */
    trap    #0                      /* syscall */
    movem.l (%sp)+, %a1/%d0-%d5     /* END OF PRINT: restore registers */
    rts
